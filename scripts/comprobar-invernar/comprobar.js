...
// También recibe un aviso de texto en POST /carrefour (lo manda
...
function panel() {
  ...
  http.createServer((req, res) => {
    ...
    if (req.method === 'POST' && req.url === '/carrefour') {
      let body = '';
      req.on('data', (chunk) => { body += chunk; });
      req.on('end', () => {
        ultimoCarrefour = body.trim() || '(aviso vacío)';
        log(ultimoCarrefour);
...
<h2 style="font-size:1rem;margin-top:1.5rem">Precios de Carrefour</h2>
<p style="font-size:1.1rem">${esc(ultimoCarrefour)}</p>
