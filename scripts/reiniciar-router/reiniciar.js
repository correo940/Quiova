...
async function reiniciarRouter() {
  ...
  try {
    ...
    if (!caido) {
      ...
      await avisarChat('Router: FALLÓ - pulsé los botones pero el router sigue respondiendo ⚠️');
      return;
    }
    log('El router ha dejado de responder. Espero a que vuelva…');
    for (let s = 0; s < 60; s++) {
      if (await responde()) {
        log('REINICIO CONFIRMADO: el router se apagó y ha vuelto');
        await avisarChat('Router: reiniciado correctamente ✅');
        return;
      }
      await new Promise((r) => setTimeout(r, 5000));
    }
    log('AVISO: el router se apagó pero no ha vuelto en 5 minutos');
    await avisarChat('Router: FALLÓ - se apagó pero no ha vuelto en 5 minutos ⚠️');
  } catch (e) {
    await foto('error');
    log('ERROR: ' + e.message);
    await avisarChat('Router: FALLÓ - ' + e.message);
  } finally {
    await browser.close();
  }
}
