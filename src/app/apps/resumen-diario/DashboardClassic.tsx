...
export default function DashboardClassic() {
    ...
    const loadData = async () => {
        try {
            ...
            expenses?.forEach((e: any) => {
                ...
                if (e.paid_by && e.paid_by.length > 10) userIds.add(e.paid_by);
            });
            ...
            allUserIdsArr.forEach(u1 => { debtMatrix[u1] = {}; allUserIdsArr.forEach(u2 => { if (u1 !== u2) debtMatrix[u1][u2] = 0; }); });
            ...
            allUserIdsArr.forEach(otherId => {
                ...
                if (Math.abs(net) > 0.1) { myDebts.push({ id: otherId, name: nameMap[otherId] || 'Compañero', amount: net }); totalBalance -= net; }
            });
            setDebts(myDebts);
            ...
            const getAlertsForDate = (date: Date) => {
                ...
                meds?.forEach((m: any) => { if (m.expiration_date && isSameDay(parseISO(m.expiration_date), date)) dateAlerts.push({ type: 'med', label: `Caduca: ${m.name}`, icon: Pill, color: 'text-red-500' }); });
                wrnties?.forEach((w: any) => { const expiry = addMonths(parseISO(w.purchase_date), w.warranty_months); if (isSameDay(expiry, date)) dateAlerts.push({ type: 'warranty', label: `Garantía: ${w.product_name}`, icon: Receipt, color: 'text-purple-500' }); });
                ins?.forEach((i: any) => { if (i.expiration_date && isSameDay(parseISO(i.expiration_date), date)) dateAlerts.push({ type: 'insurance', label: `Seguro: ${i.name}`, icon: Shield, color: 'text-orange-500' }); });
                return dateAlerts;
            };
            ...
            const processDays = (count: number) => {
                ...
                for (let i = 0; i < count; i++) {
                    ...
                    days.push({ date: d, tasks: tasks?.filter(t => isSameDay(new Date(t.due_date), d)) || [], shifts: shifts?.filter(s => isSameDay(new Date(s.start_time), d)) || [], alerts: getAlertsForDate(d) });
                }
                return days;
            };
    ...
    const expiringWarranties = warranties.filter(w => { const expiry = addMonths(parseISO(w.purchase_date), w.warranty_months); return differenceInDays(expiry, new Date()) <= 30; });
                    ...
                    <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                        <Card className="md:col-span-2 shadow-sm border-t-4 border-t-blue-500">
                            <CardHeader className="pb-3">
                                <CardTitle className="flex items-center gap-2 text-blue-700 dark:text-blue-400"><CalendarIcon className="h-5 w-5" /> Agenda de Hoy</CardTitle>
                                ...
                                <div className="space-y-6">
                                    {showCat('shifts') && (
                                        <div className="space-y-2">
                                            ...
                                            {todayData.shifts?.length > 0 ? todayData.shifts.map((shift: any) => (
                                                <div key={shift.id} className="flex items-center p-3 bg-blue-50/50 dark:bg-blue-900/20 rounded-lg border border-blue-100 dark:border-blue-800">
                                                    ...
                                                    <div><p className="font-semibold text-gray-900 dark:text-gray-100">{shift.title}</p><p className="text-sm text-gray-500">{format(new Date(shift.start_time), 'HH:mm')} - {format(new Date(shift.end_time), 'HH:mm')}</p></div>
                                                </div>
                                                ...
                                                <div className="space-y-2">
                                                    {todayData.tasks.map((task: any) => (
                                                        <div key={task.id} className="flex items-start gap-2 group p-2 hover:bg-gray-50 dark:hover:bg-gray-800 rounded transition-colors cursor-pointer">
                                                            <div className="h-4 w-4 rounded border border-gray-300 dark:border-gray-600 flex items-center justify-center mt-0.5"><div className="h-2 w-2 rounded-sm bg-transparent group-hover:bg-green-500 transition-colors" /></div>
                                    ...
                                    {showCat('shopping') && (
                                        <div className="space-y-2">
                                            ...
                                            {shoppingList.length > 0 ? (
                                                <div className="space-y-1">{shoppingList.map((item: any) => (<div key={item.id} className="text-sm text-gray-600 dark:text-gray-300 flex items-center gap-2 pl-2"><span className="w-1.5 h-1.5 rounded-full bg-orange-400" />{item.name}</div>))}</div>
                                                ...
                                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                                    {dailyMedicines.map((med: any) => (
                                                        <div key={med.id} className="p-2 border rounded bg-pink-50/50 dark:bg-pink-900/10 text-sm">
                                                            ...
                                                            <div className="flex flex-wrap gap-1 mt-1">{med.alarm_times?.map((time: string, idx: number) => (<Badge key={idx} variant="outline" className="text-xs border-pink-200 text-pink-600">{time}</Badge>))}</div>
                                                        </div>
                                                    ))}
                                                </div>
                        ...
                        {(showCat('money') || showCat('expenses')) && (
                            <Card className="shadow-sm border-t-4 border-t-green-500 lg:col-span-1 h-fit">
                                <CardHeader className="pb-3"><CardTitle className="flex items-center gap-2 text-green-900 dark:text-green-400"><Wallet className="h-5 w-5" /> Finanzas</CardTitle></CardHeader>
                                                    ...
                                                    {debts.map(debt => (
                                                        <div key={debt.id} className={`p-2 rounded border flex flex-col ${debt.amount > 0 ? 'bg-red-50 border-red-100' : 'bg-green-50 border-green-100'}`}>
                                                            <div className="flex justify-between items-center">
                                                                <span className="font-medium text-sm flex items-center gap-1 text-gray-700"><User className="w-3 h-3" /> {debt.name}</span>
                                                                <span className={`font-bold text-sm ${debt.amount > 0 ? 'text-red-600' : 'text-green-600'}`}>{Math.abs(debt.amount).toFixed(2)}€</span>
                                                            </div>
                                                            <span className="text-[10px] text-gray-500 text-right">{debt.amount > 0 ? 'Debes a esta persona' : 'Te debe a ti'}</span>
                                                        </div>
                                                    ))}
                                                </div>
                                            ) : (
                                                <div className="p-3 bg-gray-50 dark:bg-gray-800 rounded text-center"><p className="text-sm text-gray-500 italic">Cuentas saldadas ✨</p></div>
                                            )}
                                        </div>
                                    )}
                                </CardContent>
                        ...
                        {(showCat('vehicles') || showCat('medicines') || showCat('insurances') || showCat('warranties')) && (
                            <Card className="md:col-span-2 lg:col-span-3 shadow-sm border-t-4 border-t-orange-500">
                                <CardHeader className="pb-3"><CardTitle className="flex items-center gap-2 text-orange-700 dark:text-orange-400"><AlertTriangle className="h-5 w-5" /> Avisos y Mantenimiento</CardTitle></CardHeader>
                                <CardContent>
                                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                                        {showCat('vehicles') && (
                                            <div className="space-y-2">
                                                ...
                                                <ScrollArea className="h-[100px] w-full rounded-md border p-2">
                                                    {vehicleAlerts.length > 0 ? vehicleAlerts.map((v: any) => (
                                        ...
                                        {showCat('medicines') && (
                                            <div className="space-y-2">
                                                ...
                                                <ScrollArea className="h-[100px] w-full rounded-md border p-2">
                                                    {expiringMeds.length > 0 ? expiringMeds.map((m: any) => (
                                        ...
                                        {showCat('insurances') && (
                                            <div className="space-y-2">
                                                ...
                                                <ScrollArea className="h-[100px] w-full rounded-md border p-2">
                                                    {expiringInsurances.length > 0 ? expiringInsurances.map((i: any) => (
                                        ...
                                        {showCat('warranties') && (
                                            <div className="space-y-2">
                                                ...
                                                <ScrollArea className="h-[100px] w-full rounded-md border p-2">
                                                    {expiringWarranties.length > 0 ? expiringWarranties.map((w: any) => (
        ...
        <Card className={`h-full flex flex-col ${isToday ? 'border-t-4 border-t-blue-500 shadow-md ring-1 ring-blue-500/10' : 'border-t-4 border-t-gray-200 dark:border-t-gray-700'}`}>
            ...
            <CardContent className="space-y-3 px-4 pb-4 flex-1">
                {showCat('shifts') && (
                    <div className="space-y-1">
                        <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider flex items-center gap-1"><Briefcase className="h-3 w-3" /> Turno</p>
                        {dayData.shifts?.length > 0 ? dayData.shifts.map((s: any) => (
                ...
                {(visibleAlerts.length > 0 || isToday) && (
                    <div className="space-y-1">
                        ...
                        {visibleAlerts.length > 0 ? visibleAlerts.map((a: any, i: number) => { const Icon = a.icon; return <div key={i} className={`text-xs flex items-center gap-1 font-medium p-1 rounded bg-gray-50 dark:bg-gray-800 ${a.color}`}><Icon className="h-3 w-3" /><span className="truncate">{a.label}</span></div>; }) : (isToday ? <p className="text-xs text-gray-400 italic pl-1">Sin alertas</p> : null)}
                    </div>
                )}
